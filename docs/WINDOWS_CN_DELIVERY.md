...
- Windows 客户端从公开仓库 `IceTomao/backspace-cn` 的 GitHub Releases 检查更新。发现新版后先提示，只有点击“下载更新”才会开始下载；下载完成后可立即重启安装，也可在下次正常退出时安装。
...
先从托盘退出旧版，再运行 1.3.2 安装包并沿用原安装目录。不要先卸载并删除用户数据。应用身份、用户数据目录和默认中文服务器保持不变。1.3.2 是首个使用中文仓库更新源的基线版本。

本次不安装或覆盖正在运行的客户端；覆盖安装后账号及已有设置的真实保留情况仍需人工验收。旧版 1.3.0、1.3.1 交付目录保留。

## Windows 自动更新

- 启动约 10 秒后检查一次，之后每 4 小时检查，也可在设置、托盘菜单或应用菜单中手动检查。
- 发现高于当前版本的稳定版本时只提示，不自动下载。点击“下载更新”后显示进度；完成后点击“重启安装”，或正常退出后由 updater 安装。
- 检查或下载失败时可打开 `https://github.com/IceTomao/backspace-cn/releases/latest` 手动下载安装包。
- 更新源不使用访问令牌。仓库和 Release 必须保持公开，客户端才能读取 `latest.yml`、安装包和 blockmap。
- 发布 `1.3.2` 时提交当前源码并推送 `v1.3.2` 标签；以后先修改 `packages/desktop/package.json` 的版本，再推送完全匹配的 `v<version>` 标签。
- 标签工作流仅构建 Windows x64。它先创建 Draft Release，生成并校验安装包、blockmap、`latest.yml`、包内 `app-update.yml` 和 Electron 安全熔丝，全部通过后才公开；失败时保留草稿。
- 本次代码修改没有推送标签、创建 GitHub Release 或发布安装包。首次公开基线及后续升级流程仍需在仓库中执行和验收。

## 表情与我的收藏（1.3.2 覆盖更新，2026-09-22）

- Windows 表情面板保留最近使用、Smileys & People、Animals & Nature，其他分类及其搜索结果不再显示。原有独立 GIF 搜索功能保留。
- 输入框表情面板底部新增爱心页签“我的收藏”；消息回应面板只精简 Emoji 分类，不加入图片收藏。
- 收藏第一格为加号，支持多选 PNG、JPG、WebP、GIF。单张上限 20 MiB，解码像素上限 4000 万，原始图片及动画不转换。新图片排在前面，按文件内容去重，收藏图片右键可移除。
- 聊天图片右键第一项为“添加到表情”，在“保存图像”上方。附件使用现有原图预览动作解析原图地址；无法确认原图时提示失败，不猜测原文件名。头像、视频预览和链接缩略图不提供收藏入口。
- 点击收藏图片先进入当前会话附件待发送区，保留文字草稿及回复，继续使用原有上传和发送按钮，不自动发送。和普通附件一样，加入待发送区后即可能开始上传，但不会立即发送消息。
- 收藏副本及索引位于 `%APPDATA%\Backspace\favorite-images`，按登录服务器来源和账号 ID 隔离；只保存在本机，不跨电脑同步。注销不删除收藏，覆盖安装沿用原用户数据。清除用户数据或卸载时删除用户数据会使收藏丢失。
- 不保存账号密码或登录令牌到收藏目录。网页不获得任意文件路径或通用文件系统接口。服务器网页、后端、数据库和安卓代码未修改，也未部署或发布新版本。
- 本功能是客户端适配层，依赖现有 Emoji Mart 5.x、消息菜单及附件输入结构；服务端大幅更新界面后需重新检查兼容性。不能确认结构时停用相应新增入口。

本轮验证范围：桌面 TypeScript 编译和 sandbox preload 打包；针对性静态源码与差异检查；通过未打包 Electron 在真实账号界面检查 Emoji 与收藏页的导航、背景和切换恢复。未运行单元测试、前端构建或安装体验验收，不将先前版本记录视为本次功能通过依据。

人工验收清单：

- 浅色、深色及不同界面比例下的爱心入口、网格、加号、底部页签；保留 GIF 入口时无裁切或遮挡。
- 多图添加、取消选择、动图预览与发送、重复添加、右键移除、超过 20 MiB、损坏文件和网络失败提示。
- 自己及他人的图片右键菜单顺序、缩略图对应原图收藏、原消息删除后收藏仍可使用。
- 加入待发送区、草稿及回复保留、主动点击发送、上传失败和移除待发送附件。
- 切换会话或账号时的异步取消、不同账号隔离、注销重新登录、重启和 1.3.2 覆盖安装后的收藏保留。

## 文件

- `Backspace-CN-1.3.2-win-x64.exe`：中文安装程序，允许选择安装位置。
- `Backspace-CN-1.3.2-win-x64.exe.blockmap`：增量更新索引，由客户端 updater 使用。
- `latest.yml`：Windows 稳定版更新清单，必须与安装包和 blockmap 一同发布。
- `Backspace-CN-1.3.2-source.zip`：本次修改对应的完整工作区源码快照，包含许可证和构建脚本，不包含运行密钥、用户数据和依赖目录。
- `SHA256SUMS.txt`：安装包、blockmap、更新清单、源码包及本说明的 SHA-256。

可用 PowerShell 的 `Get-FileHash -Algorithm SHA256` 检查文件。

## 前次交付（1.3.2 主题设置入口，2026-09-21）
...
无需生成安装包即可运行 Windows 客户端进行人工测试。先从托盘完全退出已安装客户端，再在源码根目录运行：

```powershell
pnpm --filter @backspace/desktop dev:win
```

开发版沿用 `%APPDATA%\Backspace` 的账号、设置和收藏数据，并受单实例锁约束。修改桌面代码后需退出并重新运行该命令；确认完成后再执行下面的安装包构建命令。
...
确认功能后可运行 Windows 1.3.2 构建及交付命令；交付脚本会检查包内更新源、资源、Sharp 原生解码文件、默认配置与安全开关，并复制安装包、blockmap 和 `latest.yml`，生成交付文件及 SHA-256，不启动应用或安装程序。

`build:ts` 现在包含 preload 打包步骤，不能只运行裸 `tsc` 后直接发布，否则 sandbox preload 无法加载拆分模块。Sharp 原生模块及 DLL 必须随包解包至 `app.asar.unpacked`；图标使用 Lucide，随包保留其许可证。
